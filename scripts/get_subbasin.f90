      program calc_subbasin
! ================================================
      implicit none
! ===================
! calculation type
      character*256       ::  buf, mapname, camadir, outdir
...
! read parameters from arguments
      print *, '######################################################'
      print *, 'USAGE: Input Uparea Threshold [km2] :   ./get_subbasin $MAP $CaMadir $Outdir $UPA_THRS'
      ...
      fparam=trim(camadir)//"/map/"//trim(mapname)//"/params.txt"
...
! ===================
      ...
      finp=trim(adjustl(camadir))//"/map/"//trim(mapname)//"/nextxy.bin"
      open(11,file=finp,form='unformatted',access='direct',recl=4*nx*ny,status='old',iostat=ios)
      read(11,rec=1) nextx
      read(11,rec=2) nexty
      close(11)

      finp=trim(adjustl(camadir))//"/map/"//trim(mapname)//"/uparea.bin"
      open(11,file=finp,form='unformatted',access='direct',recl=4*nx*ny,status='old',iostat=ios)
...
! ==========
	  ...
	  !fout='../subbsn.bin'
	  fout=trim(adjustl(outdir))//"/subbasin_"//trim(mapname)//".bin"
      open(11,file=fout,form='unformatted',access='direct',recl=4*nx*ny)
      write(11,rec=1) basin
      close(11)

	  !fout='../subcol.bin'
	  fout=trim(adjustl(outdir))//"/subcolor_"//trim(mapname)//".bin"
      open(11,file=fout,form='unformatted',access='direct',recl=4*nx*ny)
